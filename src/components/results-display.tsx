...
import { Download, Database, CheckCircle, Info, ListChecks, FileJson } from 'lucide-react';
...
export function ResultsDisplay({ 
  ...
  const handleDownloadJson = () => {
    ...
    toast({
      title: "JSON Exported",
      description: "The results have been downloaded as a JSON file.",
      variant: "default",
    });
  };
  ...
  const handleSaveToFirebase = () => {
    toast({
      ...
      description: "Saving to Firebase is not implemented in this POC.",
      variant: "default",
    });
  };
    ...
    <Card className="shadow-lg">
      <CardHeader>
        <CardTitle className="font-headline text-2xl flex items-center">
          <CheckCircle className="mr-2 h-6 w-6 text-green-500" /> Processing Results
        </CardTitle>
        <CardDescription>Review the extracted insights and export the data.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {summary?.summary && (
          <div>
            <h3 className="font-semibold text-lg flex items-center mb-2"><Info className="mr-2 h-5 w-5 text-primary" />Summary</h3>
        ...
        <div>
            <h3 className="font-semibold text-lg flex items-center mb-2"><Search className="mr-2 h-5 w-5 text-primary" />Keyword Analysis</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <h4 className="font-medium text-md mb-1">Your Keywords</h4>
                ...
                <div>
                    <h4 className="font-medium text-md mb-1">Keywords Found in Text</h4>
                    ...
                    ) : <p className="text-sm text-muted-foreground">None of your keywords were found.</p>}
                </div>
            </div>
        </div>
        ...
        {enrichedKeywords?.suggestedKeywords && enrichedKeywords.suggestedKeywords.length > 0 && (
          <div>
            <h3 className="font-semibold text-lg flex items-center mb-2"><ListChecks className="mr-2 h-5 w-5 text-primary" />Suggested Keywords</h3>
             <ScrollArea className="h-32 rounded-md border p-3 bg-muted/50">
                <ul className="list-disc list-inside pl-2 text-sm space-y-1">
        ...
        <div>
            <h3 className="font-semibold text-lg flex items-center mb-2"><FileJson className="mr-2 h-5 w-5 text-primary" />Structured JSON Output</h3>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t">
          ...
          <Button onClick={handleSaveToFirebase} variant="outline" className="w-full sm:w-auto">
            <Database className="mr-2 h-4 w-4" /> Save to Firebase (POC Stub)
          </Button>
        </div>
      </CardContent>
    </Card>
...
// Add Search icon to lucide imports if not already there
